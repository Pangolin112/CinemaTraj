...
from extrinsic2pyramid.util.camera_pose_visualizer import CameraPoseVisualizer, visualize_trajectory_interactive
...
def draw_json(c2ws, vis_path):
    ...
    # Close all previous figures to ensure only one window opens
    plt.close('all')
    print("\nOpening interactive 3D visualization window...")
    print("Controls: Drag to rotate | Scroll to zoom | Use buttons for preset views")
    visualize_trajectory_interactive(c2ws)
...
def process_data(opt, output_dir, name, text=None, text_path=None, image_path=None, depth_path=None):
    ...
    for i in range(opt.test_repeat): # only once
        t0 = time.time()
